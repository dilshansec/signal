...
function selectIncident(id) {
  ...
  const btnUpper = document.getElementById('btn-upper-see-more');
  const textUpper = document.getElementById('upper-see-more-text');

  const inc = VALID_INCIDENTS.find(i => i.id === id);
  if (!inc) {
    const tooltip = document.getElementById('map-tooltip');
    if (tooltip) tooltip.style.display = 'none';
    if (btnUpper) btnUpper.style.display = 'none';
    return;
  }

  if (btnUpper && textUpper) {
    textUpper.textContent = `SEE MORE DETAILS — ${inc.name}`;
    btnUpper.style.display = 'flex';
  }
...
// Upper-middle orange SEE MORE button click handler
const btnUpperSeeMore = document.getElementById('btn-upper-see-more');
if (btnUpperSeeMore) {
  btnUpperSeeMore.addEventListener('click', (e) => {
    e.stopPropagation();
    if (selectedIncidentId) {
